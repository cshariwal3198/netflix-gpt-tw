                ...
                </div>
                {
                    hover ? (
                        <div onMouseLeave={toggleHover} className="absolute top-20 overflow-hidden right-4 p-3 z-10 flex flex-col w-fit h-1/4 justify-center items-center gap-5 rounded-xl bg-white dark:bg-black">
                            ...
                            <span className={styles} onClick={openPopupUp}>Sign Out</span>
                        </div>
                    ) : null
                }
            </div>
            {
                hidden ? null : <Popup message="Are you sure want to Sign Out?" type="warning" onPositiveAction={onSignOutClick} onNegativeAction={() => setHidden(true)} />
            }
        </>
    )
});